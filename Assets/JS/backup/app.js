...
function referenceArray (item, index) {
...
function storeFoodStuffs(response) {
  ...
  for (z = 0; z <= results_limit; z++) {
    ...
    restaurantNamesPulled.push(response.restaurants[0].restaurant.name);
    console.log("[DEBUG] :: storeFoodStuffs restaurant name stored")
    restaurantAddressPulled.push(response.restaurants[0].restaurant.location);
    console.log("[DEBUG] :: storeFoodStuffs restaurant address stored")
    restaurantPhotosPulled.push(response.restaurants[0].restaurant.photos[0].photo.thumb_url);
...
function displayFoodStuffs() {
  ...
  for (y = 0; y < restaurantNamesPulled.length; y++) {
    ...
    var cardTitleId = "#card" + y + "title";
    var cardLocationId = "#card" + y + "location";

    $(cardTitleId).text(restaurantNamesPulled[0]);
    $(cardLocationId).text(restaurantAddressPulled[0].address);
